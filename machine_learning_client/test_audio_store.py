...
def test_init_success(mock_mongo):
    mock_client, mock_db, mock_gridfs = mock_mongo
...
def test_init_with_custom_collection(mock_mongo):
    _, mock_db, mock_gridfs = mock_mongo
...
def audio_store(mock_mongo):
    mock_client, mock_db, _ = mock_mongo
    return AudioStore("mongodb://localhost:27017", "test_db")


def test_save_audio_basic(audio_store, mock_mongo):
    _, mock_db, _ = mock_mongo
    mock_fs = Mock()
    audio_store._fs = mock_fs

    file_id = ObjectId()
    mock_fs.put.return_value = file_id
    mock_attempts_col = Mock()
    mock_db.__getitem__.return_value = mock_attempts_col

    file_obj = BytesIO(b"fake audio data")
    result_id = audio_store.save_audio(file_obj, spell="Lumos", filename="test.wav", content_type="audio/wav")
...
def test_save_audio_with_score(audio_store, mock_mongo):
    _, mock_db, _ = mock_mongo
    mock_fs = Mock()
    audio_store._fs = mock_fs
    ...
    file_obj = BytesIO(b"fake audio data")
    result_id = audio_store.save_audio(file_obj, spell="Lumos", filename="test.wav", score=85.5)
...
def test_save_audio_with_transcript(audio_store, mock_mongo):
    _, mock_db, _ = mock_mongo
    mock_fs = Mock()
    audio_store._fs = mock_fs
    ...
    file_obj = BytesIO(b"fake audio data")
    result_id = audio_store.save_audio(file_obj, spell="Lumos", filename="test.wav", transcript="Lumos")
...
def test_save_audio_with_extra_metadata(audio_store, mock_mongo):
    _, mock_db, _ = mock_mongo
    mock_fs = Mock()
    audio_store._fs = mock_fs
    ...
    result_id = audio_store.save_audio(file_obj, spell="Lumos", filename="test.wav", extra_metadata=extra_metadata)
...
def test_delete_audio(audio_store, mock_mongo):
    _, mock_db, _ = mock_mongo
    mock_fs = Mock()
    audio_store._fs = mock_fs
    ...
    file_id = ObjectId()
    audio_store.delete_audio(file_id)

    mock_fs.delete.assert_called_once_with(file_id)
    mock_attempts_col.delete_many.assert_called_once_with({"audio_file_id": file_id})


def test_get_attempts_by_spell(audio_store, mock_mongo):
    _, mock_db, _ = mock_mongo
    mock_attempts_col = Mock()
    mock_db.__getitem__.return_value = mock_attempts_col
    ...
    attempts = audio_store.get_attempts_by_spell("Lumos")
